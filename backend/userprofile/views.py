...
@api_view(['GET'])
def get_products_by_catid(request,slug,id):
    profile=get_object_or_404(UserProfile,website_url=slug)
    if id!=0:
        products=Product.objects.filter(category__in=ProductCategory.objects.filter(user=profile.user,id=id))
    else:
        products=Product.objects.filter(category__in=ProductCategory.objects.filter(user=profile.user))
...
def get_website_by_slug(request,slug):
    profile=get_object_or_404(UserProfile,website_url=slug)
    categories=ProductCategory.objects.filter(user=profile.user)
    return Response({'profile':UserProfileSerializer(profile).data,'categories':ProductCategorySerializer(categories,many=True).data}, status=status.HTTP_200_OK)
